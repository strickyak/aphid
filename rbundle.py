...
class RBundleServer(rpc2.Server):
  ...
  def RStat3(bund, path):
    say bund, path
    return .bundles[bund].Stat3(path)

  def RList4(bund, path):
    say bund, path
    return list(.bundles[bund].List4(path))

  def RReadFile(bund, path, rev):
    say bund, path, rev
    return .bundles[bund].ReadFile(path, rev)

  def RWriteFile(bund, path, data, mtime, rev=None, slave=None):
    say bund, path, mtime, len(data), rev, slave
    return .bundles[bund].WriteFile(path, data, mtime, rev=rev, slave=slave)
